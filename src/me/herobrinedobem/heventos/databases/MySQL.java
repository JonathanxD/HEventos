...
public class MySQL {
	...
	public int getWins(final String player) {
		try {
			Class.forName("com.mysql.jdbc.Driver");
			final String sql = "SELECT wins FROM eventos WHERE player='" + player + "' ORDER BY wins DESC LIMIT 10";
	...
	public int getParticipations(final String player) {
		try {
			Class.forName("com.mysql.jdbc.Driver");
			final String sql = "SELECT participations FROM eventos WHERE player='" + player + "' ORDER BY participations DESC LIMIT 10";
			final ResultSet rs = this.stmt.executeQuery(sql);
			while (rs.next()) {
